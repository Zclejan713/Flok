//
//  OtherUserFlokNoImageViewController.m
//  Flok
//
//  Created by Ritwik Ghosh on 02/09/2017.
//  Copyright © 2017 Ajeet. All rights reserved.
//

#import "OtherUserFlokNoImageViewController.h"
...
@interface OtherUserFlokNoImageViewController ()
{
...
@implementation OtherUserFlokNoImageViewController
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row==0) {
        ...
        if ([arrMain count])
        {
            ...
            cell.lblTime.text=[self calculateTime:[dict valueForKey:@"comment_date"]];
            cell.tvDesc.text=[NSString stringWithFormat:@"%@",dict[@"comments"]];
            UIFont *font = [UIFont fontWithName:@"Helvetica" size:12];
            CGSize maximumSize = CGSizeMake(252, MAXFLOAT);
...
-(void)getFlokDetails{
     ...
     CLLocation *location=[Global getLocation];
    NSString *strTime=[self getLocateDate];
    //NSString *strTime=@"2016-12-28 11:55:15";
    NSString *dataString=[NSString stringWithFormat:@"user_id=%@&flok_id=%@&current_time=%@&lat=%f&lang=%f",userId,flokId,strTime,location.coordinate.latitude,location.coordinate.longitude];
    [[Global sharedInstance] setDelegate:(id)self];
...
-(void)setDataToVewpage:(NSDictionary*)dict{
    ...
    DicFlok=dict;
    distance=[dict valueForKey:@"distance"];
    ...
    int status=[[dict valueForKey:@"isLikedByMe"] intValue];
    if (status==0) {
        imgLike.image=[UIImage imageNamed:@"like"];
        
    }else{
        imgLike.image=[UIImage imageNamed:@"liked"];
    }
    ...
    if (maxLimit>4) {
        ...
        if (joined==0){
            [btnLimit setBackgroundImage:[UIImage imageNamed:@"blankLimit"] forState:UIControlStateNormal];
            ...
            [btnLimit setBackgroundImage:[UIImage imageNamed:@"halfLimit"] forState:UIControlStateNormal];
            ...
            [btnLimit setBackgroundImage:[UIImage imageNamed:@"fullLimit"] forState:UIControlStateNormal];
        }
    }else{
        if (joined==0){
